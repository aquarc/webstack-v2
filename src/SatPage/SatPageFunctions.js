...
export function getSearchPayload(state) {
    const { selectedTest, selectedTestSection, selectedSubdomains, selectedDifficulties } = state;
...
export function prepareSubdomains(selectedTestSection, selectedSubdomains, handleSubdomainChange) {
...
export class QuestionManager {
    ...
    displayQuestion(question, checkAnswerCallback) {
        ...
        if (choices.length > 0) {
          ...
          const inputContainer = this.createFreeResponseInput(question, checkAnswerCallback);
    ...
    updateNavigationButtons() {
        ...
        if (nextButton) {
          nextButton.disabled = this.currentQuestionIndex === this.currentQuestions.length - 1;
        }
    }
}